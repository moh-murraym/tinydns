...
def make(tinydns_root):
    ...
    if return_code > 0:
        raise Exception('tinydns-data error: {}'.format(return_code))
    os.chdir(cwd)
...
class _DataLine(object):
    ...
    def set_fields(self, field_dict):
        ...
        for name in self.field_names:
            if name in field_dict:
                self.fields.append(str(field_dict[name]))
            else:
                self.fields.append('')
    ...
    def create(cls, marker, fields):
        ...
        """
        kwargs = dict(list(map(_process_fields, cls.field_names, fields)))
        return cls(**kwargs)
...
class MailExchange(_DataLine):
    ...
    """
    markers = ('@',)
    field_names = ('domain', 'ip', 'server_name', 'distance', 'ttl', 'stamp',
                   'location')

    def __init__(self, domain, server_name,  ip, distance=0, ttl='',
                 stamp='', location=''):
        self.marker = '@'
        self.set_fields(vars())
...
class Generic(_DataLine):
    ...
    """
    markers = (':',)
    field_names = ('host_name', 'record_type', 'data', 'ttl', 'stamp',
                   'location')

    def __init__(self, host_name, record_type, data, ttl='', stamp='',
                 location=''):
        self.marker = ':'
        self.set_fields(vars())
...
MARKER_CLASSES = _map_marker_classes(list(globals().values()))
